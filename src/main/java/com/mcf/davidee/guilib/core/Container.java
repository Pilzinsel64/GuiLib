...
public class Container {
     ...
     *                          and the FocusableWidgets that are inside. Say that the items in the list should go from
     *                          this container's top+2
    ...
    public List<Widget> draw(int mx, int my, int scale) {
        if (clip) {
            ...
            GL11.glScissor(
                ...
                (right - left - scrollbarWidth) * scale,
                (bottom - top) * scale);
        }
    ...
    public void setFocused(FocusableWidget f) {
        ...
        if (focusIndex != newIndex) {
            if (focusIndex != -1) focusList.get(focusIndex)
                .focusLost();
            if (newIndex != -1) focusList.get(newIndex)
                .focusGained();
    ...
    public void mouseWheel(int delta) {
        if (scrollbar != null && scrollbar.shouldRender(top, bottom)) scrollbar.shiftRelative(delta);
            ...
            boolean done = false;
            if (focusIndex != -1) done = focusList.get(focusIndex)
                .mouseWheel(delta);
            else for (Iterator<Widget> it = widgets.iterator(); it.hasNext() && !done;) done = it.next()
                .mouseWheel(delta);
        }
    }
