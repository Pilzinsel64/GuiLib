...
public abstract class TextField extends FocusableWidget {
    ...
    public void draw(int mx, int my) {
        ...
        if (cursor) {
            if (flag2) Gui.drawRect(k1, i1 - 1, k1 + 1, i1 + 1 + mc.fontRenderer.FONT_HEIGHT, -3092272);
            else mc.fontRenderer.drawStringWithShadow("_", k1, i1, color);
        }
        if (k != j) {
    ...
    public void handleClick(int mx, int my) {
        ...
        String s = mc.fontRenderer.trimStringToWidth(text.substring(charOffset), getWidth());
        setCursorPosition(
            mc.fontRenderer.trimStringToWidth(s, pos)
                .length() + charOffset);
    }
    ...
    public void writeText(String str) {
        ...
        if (this.text.length() > 0) s1 = s1 + this.text.substring(0, i);
        ...
        if (text.length() > 0 && j < text.length()) s1 = s1 + this.text.substring(j);
    ...
    public void deleteFromCursor(int amt) {
        if (text.length() > 0) {
            if (selectionEnd != cursorPosition) writeText("");
                ...
                String s = "";
                if (j >= 0) s = this.text.substring(0, j);
                if (k < this.text.length()) s = s + this.text.substring(k);
                this.text = s;
    ...
    public void setSelectionPos(int index) {
        ...
        if (index == charOffset) charOffset -= mc.fontRenderer.trimStringToWidth(this.text, width, true)
            .length();
