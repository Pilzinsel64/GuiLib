...
public class Label extends Widget {
    ...
    public Label(String text, int color, int hoverColor, boolean center, Widget... tooltips) {
        ...
        for (Widget w : tooltips) this.tooltips.add(w);
    }
    ...
    public void draw(int mx, int my) {
        ...
        if (newHover && !hover) {
            ...
            for (Widget w : tooltips) w.setPosition(mx + 3, y + height);
        }
        hover = newHover;

        if (shadow) mc.fontRenderer.drawStringWithShadow(str, x, y + 2, (hover) ? hoverColor : color);
        else mc.fontRenderer.drawString(str, x, y + 2, (hover) ? hoverColor : color);
    }
