...
public abstract class BasicScreen extends GuiScreen {
    ...
    public void drawScreen(int mx, int my, float f) {
        ...
        for (Container c : containers) overlays.addAll(c.draw(mx, my, scale));
        for (Widget w : overlays) w.draw(mx, my);
    }

    @Override
    public void updateScreen() {
        for (Container c : containers) c.update();
    }
    ...
    protected void mouseClicked(int mx, int my, int code) {
        if (code == 0) {
            ...
            for (Container c : containers) if (c != selectedContainer) c.setFocused(null);
        }
    }
    ...
    protected void mouseMovedOrUp(int mx, int my, int code) {
        if (code == 0) {
            for (Container c : containers) c.mouseReleased(mx, my);
        }
    }
    ...
    public void keyTyped(char c, int code) {
        ...
        if (!handled) unhandledKeyTyped(c, code);
    }
